...
@section('content')
    <div style="margin-bottom:20px" class="w-95 w-md-75 w-lg-60 w-xl-55 mx-auto mb-6 text-center">
        <div class="subtitle alt-font">
            <span class="text-primary"></span><span class="title">Hệ thống bệnh viện</span>
        </div>
        <h2 class="display-18 display-md-16 display-lg-14 mb-0">Lịch khám của bác sĩ</h2>
                                    ...
                                    @if (isset($scheduleByTime[$time]) && count($scheduleByTime[$time]) > 0)
                                        <td class="active">
                                            @foreach ($scheduleByTime[$time] as $doctor)
                                                ...
                                                <p>{{ $doctor->schedule->first()->start_time }} -
                                                    {{ $doctor->schedule->first()->end_time }}
                                                </p>
                                                <div class="hover">
                                                    @foreach ($scheduleByTime[$time] as $doctor)
                                                        <h4>{{ $doctor->user->name }}</h4>
