        ...
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('admin.users.edit', $user->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="exampleInputName1">Họ Tên</label>
                            <input type="text" name="name" value="{{ $user->name }}" class="form-control"
                                id="exampleInputName1" placeholder="Họ tên">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <select name="role" class="form-control" id="exampleSelectGender">
                                ...
                                <option {{ $user->role === 'manager' ? 'selected' : '' }} value="schedule_manager">Quản lý
                                </option>
                                <option {{ $user->role === 'doctor' ? 'selected' : '' }} value="doctor">Bác sĩ</option>
                                <option {{ $user->role === 'nurse' ? 'selected' : '' }} value="nurse">Y tá</option>
                                <option {{ $user->role === 'receptionist' ? 'selected' : '' }} value="receptionist">Lễ tân
                                </option>
                                <option {{ $user->role === 'patient' ? 'selected' : '' }} value="patient">Bệnh nhân
                                </option>
                            </select>
                        ...
                        <div class="form-group">
                            <label for="exampleSelectGender">Căn cước công dân (Cố định)</label>
                            @if ($user->CCCD)
                                <input type="text" name="CCCD" class="form-control" value="{{ $user->CCCD }}"
                                    readonly>
                            @else
                ...
                <span class="input-group-append">
                  <button class="file-upload-browse btn btn-primary" type="button">Upload</button>
                        ...
                        <div class="form-group">
                            ...
                            @if ($user->image)
                                <p>Ảnh hiện tại</p>
                                <img style="width: 100px; height: 100px;" src="{{ asset('storage/' . $user->image) }}"
                                    alt="{{ $user->name }}" />
                            @else
