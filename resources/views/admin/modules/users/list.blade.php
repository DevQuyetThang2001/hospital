        ...
        <div class="col-lg-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <a href="{{ route('admin.users.addUserForm') }}"
                        class="btn btn-success uppercase text-md-end text-uppercase">Thêm
                        mới</a>
                    @if (session('success'))
                                ...
                                @if (!empty($data))
                                    @foreach ($data as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->CCCD }}</td>
                                            <td>
                                                <a href="{{ route('admin.users.editUserForm', $item->id) }}"
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        Bạn chắc chắn muốn xóa <span
                                                            class="text-warning">{{ $item->name }}</span>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary"
                                                            data-dismiss="modal">Thoát</button>
                                                        <a href="{{ route('admin.users.delete', $item->id) }}"
                                                            type="button" class="btn btn-primary">Đồng
                                                            ý</a>
                                                    </div>
                                                </div>
                                            </div>
