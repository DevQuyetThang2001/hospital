...
@section('content')
    <div class="row">
        <div class="col-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">Sửa thông tin bác sĩ <span class="text-secondary">{{ $doctor->user->name }}</span>
                    </h4>
                    <form class="forms-sample" action="{{ route('admin.doctor.edit', $doctor->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="exampleInputName1">Chuyên môn</label>
                            <input type="text" name="specialization" value="{{ $doctor->specialization }}"
                                class="form-control" id="exampleInputName1" placeholder="Chuyên môn">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="license_number" value="{{ $doctor->license_number }}"
                                class="form-control" id="exampleInputEmail3" placeholder="Chứng chỉ hành nghề">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="experience_years" value="{{ $doctor->experience_years }}"
                                class="form-control" id="exampleInputPassword4" placeholder="Số năm kinh nghiệm">
                        ...
                        <div class="form-group">
                            ...
                            <select name="department_id" class="form-control" id="exampleSelectGender">
                                ...
                                @foreach ($departments as $item)
                                    <option {{ $item->id === $doctor->department_id ? 'selected' : '' }}
                        ...
                        {{-- Nội dung (CKEditor) --}}
                        <div class="form-group">
                            <label for="description">Mô tả bác sĩ</label>
                            <textarea name="description" id="editor" rows="8" class="form-control" placeholder="Nhập nội dung mô tả...">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="text-danger text-sm">{{ $message }}</div>
                            @enderror
                        </div>
                  ...
                  <div class="input-group col-xs-12">
                    ...
                    <span class="input-group-append">
                      <button class="file-upload-browse btn btn-primary" type="button">Upload</button>
                        ...
                        <a href="{{ route('admin.doctors.list') }}" type="submit" class="btn btn-primary mr-2">Quay
                            lại</a>
                    </form>
                </div>
            </div>
