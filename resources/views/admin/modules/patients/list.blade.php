    ...
    <div class="col-lg-12 stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <a href="{{route('admin.patients.addPatientForm')}}" class="btn btn-success uppercase text-md-end text-uppercase">Xác nhận</a>
          @if (session('success'))
              ...
              <tbody>
                @if(!empty($data))
                    ...
                    <tr>
                        <td>
                            @if ($item->user)
                                <img style="width: 80px;height: 80px;object-fit: cover" src="{{asset('storage/'.$item->user->image)}}" alt="{{$item->user->name}}"/>
                            @else
                        ...
                        <td>
                            <a href="{{route('admin.patient.editPatientForm', $item->id)}}" class="btn btn-secondary">
                              Sửa
                            </a>
                    ...
                    <div class="modal fade z-3" id="modal-{{ $item->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                         <div class="modal-dialog">
                            ...
                            <div class="modal-body">
                                Bạn chắc chắn muốn xóa <span class="text-warning">{{$item->user->name}}</span>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Thoát</button>
                                <a href="{{route('admin.patient.delete',$item->id)}}" type="button" class="btn btn-primary">Đồng ý</a>
                            </div>
                            </div>
                         </div>
                        </div>
